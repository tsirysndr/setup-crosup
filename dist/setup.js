    ...
    if (cacheEnabled) {
        ...
        if (cacheRestored) {
            ...
            if (version) {
                ...
                action.warning("Found a cached version of Crosup, but it appears to be corrupted? Attempting to download a new version.");
            }
        }
    }
    if (!cacheHit) {
...
function getDownloadUrl(options) {
    const release = encodeURIComponent(options?.version ?? "v0.4.10");
    const platform = {
        darwin: "apple-darwin",
    ...
    const { href } = new URL(`${release}/crosup_${release}_${cpu[arch] || arch}-${os}.tar.gz`, "https://github.com/tsirysndr/crosup/releases/download/");
    return {
        url: href,
        cacheKey: `crosup-${release}-${arch}-${platform}`,
    };
}
async function verifyCrosup(path) {
